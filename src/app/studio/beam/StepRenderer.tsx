 ...
 *   - Header with step name + code citation
 *   - KaTeX-rendered equation
 *   - Variable substitution table (each input pill color-coded by provenance)
 *   - Result line with output unit
 *
 * Variables can be clicked to navigate to the step that produced them.
 * Used by both Live mode (TS port of flexure) and Official mode (SSE stream).
 */
...
interface Props {
  ...
  index: number;
  /** Optional: a variable name to flash-highlight (e.g., the user is hovering it elsewhere). */
  highlightVar?: string;
  /** Click handler — fires with the variable name when an input pill is clicked. */
  onVarClick?: (varName: string) => void;
  /** Click handler — fires with the dependency step id when a depends_on chip is clicked. */
  onDependencyClick?: (stepId: string) => void;
  /** Click handler — fires with this step's id (e.g., for verification panel cross-reference). */
  onSelfClick?: (stepId: string) => void;
  /** Whether the step has just (re)computed — used for pulse animation. */
  recentlyChanged?: boolean;
}

function formatNumber(n: number, unit: string): string {
  if (!isFinite(n)) return "∞";
  // Choose precision based on magnitude — keeps "0.000142" readable while
  // also handling 2,230,000 kg-cm without 17-digit floats.
...
export function StepRenderer({
  ...
  const isVerification = step.id.startsWith("flexure.verif_");
  ...
  return (
    ...
    >
            ...
            return (
              <button
                ...
                className={[
                  ...
                  isHi ? "ring-2 ring-amber-400 ring-offset-1 ring-offset-zinc-950" : "",
      ...
      {/* Dependencies (only if requested via onDependencyClick) */}
